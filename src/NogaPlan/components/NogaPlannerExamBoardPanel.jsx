...
const NogaPlannerExamBoardPanel = ({
  ...
  const examEditorFormId =
    exam_form_mode === "Edit"
      ? "nogaPlanner_form_editExam"
      : "nogaPlanner_form_addExam";
  ...
  const selectedExam =
    selectedCourse && selected_exam_index >= 0
      ? planner.getRenderableCourseExamEntries(selectedCourse)[
          selected_exam_index
        ] || null
      : null;

  const linkedLectureOptions = planner.getExamLectureOptionsForCourse(
    examDraft?.selectedCourseId || "",
  );
  const examCoursesCount = courseEntries.filter(
    (course) => planner.getRenderableCourseExamEntries(course).length > 0,
  ).length;
  ...
  const resolveExamLocationBuildingValue = (examEntry) =>
    String(
      examEntry?.location?.building || examEntry?.locationBuilding || "-",
    ).trim() || "-";
  const resolveExamLocationRoomValue = (examEntry) =>
    String(
      examEntry?.location?.room || examEntry?.locationRoom || "-",
    ).trim() || "-";
  const resolveExamComponentTypeValue = (course, examEntry) => {
    ...
    const components = Array.isArray(course?.course_components)
      ? course.course_components
      : [];
    const matchedComponent = components.find(
      (componentEntry) =>
        String(componentEntry?._id || componentEntry?.id || "").trim() ===
        targetComponentId,
    );
    return (
      String(
        matchedComponent?.course_class || course?.course_class || "-",
      ).trim() || "-"
    );
  };
  ...
  const renderWrapperTabs = () => (
    <div
      id="nogaPlanner_exam_wrapperTabs"
      className="nogaPlanner_wrapperTabs nogaPlanner_exam_wrapperTabs"
    >
      {(Array.isArray(NOGAPLANNER_WRAPPER_TABS)
        ? NOGAPLANNER_WRAPPER_TABS.filter(
            (tabEntry) => String(tabEntry?.key || "").trim() !== "plan",
          )
        : []
      ).map((tabEntry) => (
        <button
  ...
  const renderExamMiniBar = () => (
    <div
      id="nogaPlanner_exam_miniBar"
      className="nogaPlanner_coursesMiniBar nogaPlanner_exam_miniBar"
    >
      <div
        id="nogaPlanner_exam_miniBar_tabs"
        className="nogaPlanner_coursesMiniBarCol nogaPlanner_coursesMiniBarCol--tabs nogaPlanner_exam_miniBarCol nogaPlanner_exam_miniBarCol--tabs"
      >
        {renderWrapperTabs()}
      </div>
      <div
        id="nogaPlanner_exam_miniBar_actions"
        className="nogaPlanner_coursesMiniBarCol nogaPlanner_coursesMiniBarCol--actions nogaPlanner_exam_miniBarCol nogaPlanner_exam_miniBarCol--actions"
      >
        <div
          id="nogaPlanner_exam_miniBar_actionsGroup"
          className="nogaPlanner_coursesMiniBarGroup nogaPlanner_coursesMiniBarGroup--editDelete nogaPlanner_exam_miniBarGroup"
        >
          <button
  ...
  const renderExamEditorPanel = () => (
    <form
      id={examEditorFormId}
      className="nogaPlanner_savedCourseEditor nogaPlanner_examEditor"
      data-panel-id="nogaPlanner_exam_editor"
      onSubmit={(event) => {
        event.preventDefault();
        planner.submitExamForm();
      }}
    >
      <div
        id="nogaPlanner_exam_editorGrid"
        className="nogaPlanner_savedCourseEditorGrid nogaPlanner_exam_editorGrid"
      >
        <div
          id="nogaPlanner_exam_editorCell_course"
          className="nogaPlanner_savedCourseEditorCell nogaPlanner_savedCourseEditorCell--course nogaPlanner_exam_editorCell nogaPlanner_exam_editorCell--course"
        >
          <div
            id="nogaPlanner_exam_editorTitleRow"
            className="nogaPlanner_formCardTitleRow nogaPlanner_exam_editorTitleRow"
          >
            <p
              id="nogaPlanner_exam_editorLinkedCourseLabel"
              className="nogaPlanner_savedCourseEditorLabel nogaPlanner_exam_editorLabel"
            >
              {EXAM_UI_TEXT.linkedCourseLabel}
            </p>
          ...
          <input
            ...
            onChange={(event) =>
              planner.handleExamDraftChange("type", event.target.value)
            }
        ...
        <div
          id="nogaPlanner_exam_editorCell_details"
          className="nogaPlanner_savedCourseEditorCell nogaPlanner_savedCourseEditorCell--componentDetails nogaPlanner_exam_editorCell nogaPlanner_exam_editorCell--details"
        >
          <p
            id="nogaPlanner_exam_editorTimingLabel"
            className="nogaPlanner_savedCourseEditorLabel nogaPlanner_exam_editorLabel"
          >
            {EXAM_UI_TEXT.timingLabel}
          </p>

          <div
            id="nogaPlanner_exam_timingGroups"
            className="nogaPlanner_savedCourseDetailGroupsGrid nogaPlanner_exam_detailGroupsGrid"
          >
            <div
              id="nogaPlanner_exam_dateGroup"
              className="nogaPlanner_savedCourseDetailGroup nogaPlanner_exam_detailGroup"
            >
              <span className="nogaPlanner_savedCourseDetailGroupTitle nogaPlanner_exam_detailGroupTitle">
                {EXAM_UI_TEXT.dateLabel}
              </span>
              <input
                className="nogaPlanner_savedCoursesDetailsInput"
                type="date"
                value={examDraft.exam_date}
                onChange={(event) =>
                  planner.handleExamDraftChange("exam_date", event.target.value)
                }
                placeholder={EXAM_UI_TEXT.datePlaceholder}
              />
            </div>
            <div
              id="nogaPlanner_exam_timeGroup"
              className="nogaPlanner_savedCourseDetailGroup nogaPlanner_exam_detailGroup"
            >
              <span className="nogaPlanner_savedCourseDetailGroupTitle nogaPlanner_exam_detailGroupTitle">
                {EXAM_UI_TEXT.timeLabel}
              </span>
              <input
                className="nogaPlanner_savedCoursesDetailsInput"
                type="time"
                value={examDraft.exam_time}
                onChange={(event) =>
                  planner.handleExamDraftChange("exam_time", event.target.value)
                }
                placeholder={EXAM_UI_TEXT.timePlaceholder}
              />
            </div>
          </div>
        </div>
        ...
        <div
          id="nogaPlanner_exam_linkedLecturesGroup"
          className="nogaPlanner_savedCourseDetailGroup nogaPlanner_savedCourseEditorFieldFull nogaPlanner_exam_detailGroup nogaPlanner_exam_detailGroup--full"
        >
          <span className="nogaPlanner_savedCourseDetailGroupTitle nogaPlanner_exam_detailGroupTitle">
            {EXAM_UI_TEXT.linkedLecturesLabel}
          </span>
          <div
            id="nogaPlanner_exam_linkedLecturesTokens"
            className="nogaPlanner_examLectureTokens nogaPlanner_exam_linkedLecturesTokens"
          >
            {linkedLectureOptions.length === 0 ? (
              <span className="nogaPlanner_examLectureEmpty">
                {EXAM_UI_TEXT.noLinkedLectures}
              </span>
            ) : (
              linkedLectureOptions.map((lectureOption) => {
                ...
                return (
                  <button
                    id={`nogaPlanner_exam_linkedLectureToken_${lectureOption.id}`}
                    key={lectureOption.id}
                    type="button"
                    className={
                      "nogaPlanner_examLectureToken nogaPlanner_exam_linkedLectureToken" +
                      (isActive ? " is-active" : "")
                    }
                    onClick={() =>
                      planner.toggleExamDraftLectureId(lectureOption.id)
                    }
                  >
                    {lectureOption.label}
                  </button>
        ...
        <div className="nogaPlanner_savedCourseDetailGroupsGrid">
          <div className="nogaPlanner_savedCourseDetailGroup">
            ...
            <input
              ...
              onChange={(event) =>
                planner.handleExamDraftChange("volumeValue", event.target.value)
              }
              placeholder={EXAM_UI_TEXT.valuePlaceholder}
            />
            <input
              className="nogaPlanner_savedCoursesDetailsInput"
              type="text"
              ...
              onChange={(event) =>
                planner.handleExamDraftChange("volumeScope", event.target.value)
              }
              placeholder={EXAM_UI_TEXT.scopePlaceholder}
            />
            <textarea
              className="nogaPlanner_savedCoursesDetailsInput"
              value={examDraft.volumeNote}
              onChange={(event) =>
                planner.handleExamDraftChange("volumeNote", event.target.value)
              }
              placeholder={EXAM_UI_TEXT.notePlaceholder}
            />
          </div>

          <div className="nogaPlanner_savedCourseDetailGroup">
            <span className="nogaPlanner_savedCourseDetailGroupTitle">
              {EXAM_UI_TEXT.weightLabel}
            </span>
            <input
              className="nogaPlanner_savedCoursesDetailsInput"
              type="number"
              ...
              onChange={(event) =>
                planner.handleExamDraftChange("weightValue", event.target.value)
              }
              placeholder={EXAM_UI_TEXT.valuePlaceholder}
            />
          </div>
        ...
        <div className="nogaPlanner_savedCourseDetailGroupsGrid">
          <div className="nogaPlanner_savedCourseDetailGroup">
            ...
            <input
              ...
              placeholder={EXAM_UI_TEXT.minPlaceholder}
            />
            <input
              className="nogaPlanner_savedCoursesDetailsInput"
              type="number"
              ...
              placeholder={EXAM_UI_TEXT.maxPlaceholder}
            />
          </div>

          <div className="nogaPlanner_savedCourseDetailGroup">
            <span className="nogaPlanner_savedCourseDetailGroupTitle">
              {EXAM_UI_TEXT.gradeLabel}
            </span>
            <input
              className="nogaPlanner_savedCoursesDetailsInput"
              type="number"
              ...
              onChange={(event) =>
                planner.handleExamDraftChange("gradeValue", event.target.value)
              }
              placeholder={EXAM_UI_TEXT.valuePlaceholder}
            />
            <input
              ...
              onChange={(event) =>
                planner.handleExamDraftChange("gradeMin", event.target.value)
              }
              placeholder={EXAM_UI_TEXT.minPlaceholder}
            />
            <input
              ...
              onChange={(event) =>
                planner.handleExamDraftChange("gradeMax", event.target.value)
              }
              placeholder={EXAM_UI_TEXT.maxPlaceholder}
            />
          </div>
        ...
        <div
          id="nogaPlanner_exam_editorActions"
          className="nogaPlanner_savedCourseEditorActionRow nogaPlanner_exam_editorActions"
        >
          <button
            id="nogaPlanner_exam_editorBtn_submit"
            type="submit"
            className="nogaPlanner_coursesMiniBarBtn nogaPlanner_exam_editorBtn"
          >
            {exam_form_mode === "Edit"
              ? EXAM_UI_TEXT.saveExam
              : EXAM_UI_TEXT.addExam}
          </button>
          <button
            id="nogaPlanner_exam_editorBtn_cancel"
            type="button"
            className="nogaPlanner_coursesMiniBarBtn nogaPlanner_exam_editorBtn"
            onClick={planner.closeAddExamForm}
          >
  ...
  const renderExamTableBlock = () => (
    <div
      id="nogaPlanner_exam_tableBody"
      className="nogaPlanner_savedCoursesColumnBody nogaPlanner_exam_tableBody"
      ref={planner.savedCoursesColumnBodyRef}
    >
      <div
        id="nogaPlanner_exam_tableBodyScroll"
        className="nogaPlanner_savedCoursesTableBodyScroll nogaPlanner_exam_tableBodyScroll"
        onScroll={planner.handleSavedCoursesBodyScroll}
      >
        <table
          id="nogaPlanner_exam_table"
          className="nogaPlanner_tabTable nogaPlanner_savedCoursesTable nogaPlanner_exam_table"
        >
          <thead id="nogaPlanner_exam_tableHead" className="nogaPlanner_tableHead">
            <tr id="nogaPlanner_exam_tableHead_row_1">
              <th id="nogaPlanner_exam_th_courseName" rowSpan={2}>
                <span
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => planner.handleExamBoardSort("course_name")}
                  onKeyDown={(event) =>
                    (event.key === "Enter" || event.key === " ") &&
                    planner.handleExamBoardSort("course_name")
                  }
                >
                  {renderExamBoardSortLabel(
                    "course_name",
                    EXAM_UI_TEXT.tableCourseName,
                  )}
                </span>
              </th>
              <th id="nogaPlanner_exam_th_componentType" rowSpan={2}>
                {EXAM_UI_TEXT.tableComponentType}
              </th>
              <th id="nogaPlanner_exam_th_type" rowSpan={2}>
                <span
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => planner.handleExamBoardSort("type")}
                  onKeyDown={(event) =>
                    (event.key === "Enter" || event.key === " ") &&
                    planner.handleExamBoardSort("type")
                  }
                >
                  {renderExamBoardSortLabel("type", EXAM_UI_TEXT.tableType)}
                </span>
              </th>
              <th id="nogaPlanner_exam_th_timeGroup" colSpan={3}>
                {EXAM_UI_TEXT.tableTime}
              </th>
              <th id="nogaPlanner_exam_th_locationGroup" colSpan={2}>
                <span
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => planner.handleExamBoardSort("location")}
                  onKeyDown={(event) =>
                    (event.key === "Enter" || event.key === " ") &&
                    planner.handleExamBoardSort("location")
                  }
                >
                  {renderExamBoardSortLabel(
                    "location",
                    EXAM_UI_TEXT.tableLocation,
                  )}
                </span>
              </th>
              <th id="nogaPlanner_exam_th_volume" rowSpan={2}>
                <span
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => planner.handleExamBoardSort("volume")}
                  onKeyDown={(event) =>
                    (event.key === "Enter" || event.key === " ") &&
                    planner.handleExamBoardSort("volume")
                  }
                >
              ...
              <th id="nogaPlanner_exam_th_weight" rowSpan={2}>
                <span
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => planner.handleExamBoardSort("weight")}
                  onKeyDown={(event) =>
                    (event.key === "Enter" || event.key === " ") &&
                    planner.handleExamBoardSort("weight")
                  }
                >
                  {renderExamBoardSortLabel("weight", EXAM_UI_TEXT.tableWeight)}
                </span>
              </th>
              <th id="nogaPlanner_exam_th_pass" rowSpan={2}>
                {EXAM_UI_TEXT.tablePass}
              </th>
              <th id="nogaPlanner_exam_th_grade" rowSpan={2}>
                {EXAM_UI_TEXT.tableGrade}
              </th>
            </tr>
            <tr id="nogaPlanner_exam_tableHead_row_2">
              <th id="nogaPlanner_exam_th_date">{EXAM_UI_TEXT.tableDate}</th>
              <th id="nogaPlanner_exam_th_startTime">
                {EXAM_UI_TEXT.tableStartTime}
              </th>
              <th id="nogaPlanner_exam_th_endTime">
                {EXAM_UI_TEXT.tableEndTime}
              </th>
              <th id="nogaPlanner_exam_th_building">
                {EXAM_UI_TEXT.tableBuilding}
              </th>
          ...
          <tbody id="nogaPlanner_exam_tableBodyRows">
            {sortedExamRows.length === 0 ? (
              <tr id="nogaPlanner_exam_tableBody_emptyRow">
                <td
                  id="nogaPlanner_exam_tableBody_emptyCell"
                  colSpan={12}
                  className="nogaPlanner_savedCoursesEmpty nogaPlanner_exam_emptyCell"
                >
                  {EXAM_UI_TEXT.empty}
                </td>
              </tr>
              ...
              sortedExamRows.map(({ course, examEntry, examIndex }) => (
                <tr
                  id={`nogaPlanner_exam_tableBody_row_${course?._id || "course"}_${examIndex}`}
                  key={`${course?._id || course?.course_name || "course"}-${examIndex}`}
                  className={
                    "nogaPlanner_tabTableRow nogaPlanner_exam_tableRow" +
                    (String(course?._id || "").trim() ===
                      String(selected_course_id || "").trim() &&
                  ...
                  onClick={() =>
                    planner.setSelectedCourseWithExamFocus(
                      course?._id,
                      examIndex,
                    )
                  }
                >
                  <td
                    id={`nogaPlanner_exam_td_courseName_${course?._id || "course"}_${examIndex}`}
                    className="nogaPlanner_exam_tableCell nogaPlanner_exam_tableCell--courseName"
                  ...
                  <td
                    style={getCellAlignmentStyle(
                      resolveExamComponentTypeValue(course, examEntry),
                    )}
                  >
                    {resolveExamComponentTypeValue(course, examEntry)}
                  </td>
                  ...
                  <td
                    style={getCellAlignmentStyle(
                      resolveExamDateValue(examEntry),
                    )}
                  >
                    {resolveExamDateValue(examEntry)}
                  </td>
                  <td
                    style={getCellAlignmentStyle(
                      resolveExamStartTimeValue(examEntry),
                    )}
                  >
                    {resolveExamStartTimeValue(examEntry)}
                  </td>
                  <td
                    style={getCellAlignmentStyle(
                      resolveExamEndTimeValue(examEntry),
                    )}
                  >
                    {resolveExamEndTimeValue(examEntry)}
                  </td>
                  <td
                    style={getCellAlignmentStyle(
                      resolveExamLocationBuildingValue(examEntry),
                    )}
                  >
                    {resolveExamLocationBuildingValue(examEntry)}
                  </td>
                  <td
                    style={getCellAlignmentStyle(
                      resolveExamLocationRoomValue(examEntry),
                    )}
                  >
                    {resolveExamLocationRoomValue(examEntry)}
                  </td>
  ...
  if (editorOnly) {
    return show_addExamForm ? renderExamEditorPanel() : null;
  }
