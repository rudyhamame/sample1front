...
export default class NogaPlanner extends Component {
        ...
        Array.isArray(course?.components) && course.components.length > 0
          ? course.components.map((component) => {
              ...
              return {
                ...
                actualCourseYearNum: normalizeProgramYearValue(
                  component?.actualCourseYearNum ||
                    component?.time?.actual?.courseYearNum,
                ),
                actualCourseYearInterval: normalizeAcademicYearValue(
                  component?.actualCourseYearInterval ||
                    component?.course_year ||
                    component?.time?.actual?.courseYearInterval,
                ),
                actualCourseTerm:
                  String(
                    component?.actualCourseTerm ||
                      component?.course_term ||
                      component?.time?.actual?.courseTerm ||
                      "",
                  ).trim() === "-"
                    ? ""
                    : String(
          ...
          : [
              {
                ...
                actualCourseYearNum: normalizeProgramYearValue(
                  course?.actualCourseYearNum ||
                    course?.time?.actual?.courseYearNum,
                ),
                actualCourseYearInterval: normalizeAcademicYearValue(
  ...
  submitSavedCourseEditor = async () => {
    ...
    const componentsToPersist =
      persistedComponents.length > 0
        ...
        : pendingComponent
          ? [pendingComponent]
          : [];
    const primaryComponent = componentsToPersist[0] || null;
    ...
    if (
      !primaryComponent ||
      !String(primaryComponent?.course_class || "").trim()
    ) {
      ...
      if (componentId) {
        await fetch(
            ...
            componentId,
          {
            ...
            body: JSON.stringify({
              course_class: primaryComponent.course_class || "-",
              normativeCourseYearNum: primaryComponent.normativeCourseYearNum
                ? Number(primaryComponent.normativeCourseYearNum)
                : null,
              normativeCourseTerm: primaryComponent.normativeCourseTerm || "-",
              actualCourseYearNum: primaryComponent.actualCourseYearNum
                ? Number(primaryComponent.actualCourseYearNum)
                : null,
              actualCourseYearInterval:
                primaryComponent.actualCourseYearInterval || "-",
              actualCourseTerm: primaryComponent.actualCourseTerm || "-",
              programYear: primaryComponent.programYear
                ? Number(primaryComponent.programYear)
                : null,
              course_dayAndTime: Array.isArray(
                primaryComponent.course_dayAndTime,
              )
                ? primaryComponent.course_dayAndTime
                : [],
              course_year: primaryComponent.course_year || "-",
              academicYear: primaryComponent.course_year || "-",
              course_term: primaryComponent.course_term || "-",
              term: primaryComponent.course_term || "-",
              course_grade: primaryComponent.course_grade || "-",
              course_locationBuilding:
                primaryComponent.course_locationBuilding || "-",
              course_locationRoom: primaryComponent.course_locationRoom || "-",
            }),
          },
        );
        ...
        <div className="nogaPlanner_savedCourseEditorGrid">
          ...
          <div className="nogaPlanner_savedCourseEditorCell nogaPlanner_savedCourseEditorCell--componentDetails nogaPlanner_savedCourseEditorCell--componentBlock">
                ...
                <div className="nogaPlanner_savedCourseDetailGroup">
                  ...
                  >
                    ...
                    {ACADEMIC_YEAR_OPTIONS.map((optionValue) => (
                      <option key={`actual-${optionValue}`} value={optionValue}>
                        {optionValue}
                      </option>
                    ))}
                  </select>
            ...
            <div className="nogaPlanner_savedCourseEditorActionRow">
              ...
              >
                {this.isArabic() ? "إضافة مكوّن" : "Add component"}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="nogaPlanner_savedCoursesWorkspaceRow">
              ...
              <div className="nogaPlanner_savedCoursesColumnBody">
                {shouldShowSelectedCourseLectures ? (
                  ...
                  <>
                    <div className="nogaPlanner_savedCoursesTableHeader">
                      <table
                        className={
